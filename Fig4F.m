...
load('./data/sigma_0.5.mat', 'real_E', 'imag_E');
...
starting_point = convert_to_0_to_a(1 - 0.84 - 65.8 / 360, 1);
...
plot(sub_reg(3), 'LineWidth', 1, ...
    'EdgeColor', [1, 1, 1] * 0.3, 'FaceColor', [1, 1, 1] * 0.7); hold on;
% Winding number = 2, write out the coordinates explicitly
[~, idx_11] = find_closest(sub_reg(1).Vertices(:, 1), -0.0102980);
[~, idx_12] = find_closest(sub_reg(1).Vertices(:, 1), -0.0105263);
[~, idx_21] = find_closest(sub_reg(2).Vertices(:, 1), -0.0105263);
[~, idx_22] = find_closest(sub_reg(2).Vertices(:, 1), -0.0126243);
[~, idx_31] = find_closest(sub_reg(3).Vertices(:, 1), -0.0126243);
[~, idx_32] = find_closest(sub_reg(3).Vertices(:, 1), -0.0102980);
sub_reg_4 = polyshape( ...
...
plot(sub_reg_4, 'LineWidth', 1, ...
    'EdgeColor', [1, 1, 1] * 0.3, 'FaceColor', [1, 1, 1] * 0.7/3); hold off;
