...
load('./data/sigma_0.mat', 'real_E', 'imag_E');
...
figure;
yyaxis left;
scatter(fast_time, real_E, 20, [0 0.4470 0.7410], "filled");
ylim([-1, 1] * 0.04); yticks([-1, -0.5, 0, 0.5, 1] * 0.04);
% ylabel('Re({\itE}) / {\it\Omega_R}');
yticklabels([]);

yyaxis right;
scatter(fast_time, imag_E, 20, [0.8500 0.3250 0.0980], "filled");
ylim([-1, 1] * 0.015); yticks([-1, -0.5, 0, 0.5, 1] * 0.015);
...
%% Plot winding
figure; color_list = hsv(num_per_roundtrip);
...
pbaspect([1, 1, 1]);
xlim([-1, 1] * 0.04); ylim([-1, 1] * 0.015);
xticks([-1, 1] * 0.04); yticks([-1, 1] * 0.015);
xticklabels([]); yticklabels([]); 
% xlabel('Re({\itE}) / {\it\Omega_R}'); ylabel('Im({\itE}) / {\it\Omega_R}');
colormap hsv; set(gcf, 'unit', 'normalized', 'Position', [0.2 0.05 0.5 0.4], 'Color', 'w');